...
/******************/
-(void)setCaroArr:(NSArray *)caroArr
{
    _caroArr = caroArr;
    [_table reloadData];
}

-(void)sr_showAtPoint:(CGPoint)point tableWidth:(CGFloat)width titleArray:(NSArray *)titleArr popoverPostion:(DXPopoverPosition)position inView:(UIView *)containerView
{
    _caroArr = titleArr;
    
    _table = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, width, titleArr.count * 44) style:UITableViewStylePlain];
    _table.delegate = self;
    _table.dataSource = self;
    
    if (titleArr.count > 0) {
        [self showAtPoint:point popoverPostion:position withContentView:_table inView:containerView];
    }
}

- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return _caroArr.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    cell.textLabel.text = _caroArr[indexPath.row];
    cell.textLabel.font = LRFont(14);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self dismiss];
    if (self.block) {
        self.block(indexPath.row, _caroArr[indexPath.row]);
    }
}
